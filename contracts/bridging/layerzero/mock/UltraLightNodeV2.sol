    ...
    Ownable
{
    ...
    // User Application
    mapping(address => mapping(uint16 => ApplicationConfiguration))
        public appConfig; // app address => chainId => config
    ...
    mapping(address => mapping(uint16 => mapping(bytes32 => mapping(bytes32 => uint))))
        public hashLookup; //[oracle][srcChainId][blockhash][datahash] -> confirmation
    ...
    function validateTransactionProof(
        ...
        );
        {
            ...
            uint storedConfirmations = hashLookup[uaConfig.oracle][_srcChainId][
                _lookupHash
            ][_blockData];
            require(
                storedConfirmations > 0 &&
                    storedConfirmations >= uaConfig.inboundBlockConfirmations,
                "LayerZero: not enough block confirmations"
            );

            // decode
            address inboundProofLib = inboundProofLibrary[_srcChainId][
        ...
        require(
            ulnLookup[_srcChainId] == _packet.ulnAddress &&
                _packet.ulnAddress != bytes32(0),
            "LayerZero: invalid _packet.ulnAddress"
        );
        require(
            _packet.srcChainId == _srcChainId,
            "LayerZero: invalid srcChain Id"
        );
        // failsafe because the remoteAddress size being passed into validateProof trims the address this should not hit
        require(
            _packet.srcAddress.length == remoteAddressSize,
            "LayerZero: invalid srcAddress size"
        );
        require(
            _packet.dstChainId == localChainId,
            "LayerZero: invalid dstChain Id"
        );
        require(
            _packet.dstAddress == _dstAddress,
    ...
    function send(
    ...
    ) public virtual payable override onlyEndpoint {
        address ua = _ua;
        ...
        uint relayerFee = _handleRelayer(
            dstChainId,
            uaConfig,
            ua,
            payload.length,
        ...
        uint nativeProtocolFee = _handleProtocolFee(
            relayerFee,
            oracleFee,
            ua,
            _zroPaymentAddress
    ...
    function _handleRelayer(
        ...
        if (_adapterParams.length == 0) {
            _adapterParams = defaultAdapterParams[_dstChainId][
    ...
    function updateHash(
        ...
        uint storedConfirmations = hashLookup[msg.sender][_srcChainId][
            _lookupHash
        ][_blockData];
        ...
        hashLookup[msg.sender][_srcChainId][_lookupHash][
            _blockData
        ] = _confirmations;
    ...
    function getAppConfig(
        ...
        address _ua
    ) public virtual view override returns (ApplicationConfiguration memory) {
    ...
    function _getAppConfig(
        uint16 _remoteChainId,
        address _ua
    ) internal view returns (ApplicationConfiguration memory) {
        ApplicationConfiguration memory config = appConfig[_ua][_remoteChainId];
        ApplicationConfiguration storage defaultConfig = defaultAppConfig[
            _remoteChainId
        ];

        if (config.inboundProofLibraryVersion == 0) {
            config.inboundProofLibraryVersion = defaultConfig
                .inboundProofLibraryVersion;
        }

        if (config.inboundBlockConfirmations == 0) {
            config.inboundBlockConfirmations = defaultConfig
        ...
        if (config.outboundBlockConfirmations == 0) {
            config.outboundBlockConfirmations = defaultConfig
    ...
    function setConfig(
    ...
    ) public virtual override onlyEndpoint {
        ApplicationConfiguration storage uaConfig = appConfig[_ua][
            _remoteChainId
        ];
        if (_configType == CONFIG_TYPE_INBOUND_PROOF_LIBRARY_VERSION) {
            ...
            require(
                inboundProofLibraryVersion <=
                    maxInboundProofLibrary[_remoteChainId],
            ...
            require(
                supportedOutboundProof[_remoteChainId][outboundProofType] ||
                    outboundProofType == 0,
    ...
    function getConfig(
        ...
        address _ua,
        uint _configType
    ) public virtual view override returns (bytes memory) {
        ApplicationConfiguration storage uaConfig = appConfig[_ua][
            _remoteChainId
        ];
        ...
        if (_configType == CONFIG_TYPE_INBOUND_PROOF_LIBRARY_VERSION) {
            if (uaConfig.inboundProofLibraryVersion == 0) {
                return
                    abi.encode(
                        defaultAppConfig[_remoteChainId]
                            .inboundProofLibraryVersion
                    );
            }
    ...
    function estimateFees(
        ...
        bool _payInZRO,
        bytes calldata _adapterParams
    ) public virtual view override returns (uint nativeFee, uint zroFee) {
        ApplicationConfiguration memory uaConfig = _getAppConfig(
            _dstChainId,
            _ua
        );
        ...
        if (_adapterParams.length > 0) {
            ...
            adapterParams = defaultAdapterParams[_dstChainId][
    ...
    function withdrawZRO(
        ...
        require(
            msg.sender == address(treasuryContract),
    ...
    function setLayerZeroToken(address _layerZeroToken) public virtual onlyOwner {
        require(
            _layerZeroToken != address(0x0),
    ...
    function addInboundProofLibraryForChain(
        uint16 _chainId,
        address _library
    ) public virtual onlyOwner {
        require(
            _library != address(0x0),
    ...
    function enableSupportedOutboundProof(
        uint16 _chainId,
        uint16 _proofType
    ) public virtual onlyOwner {
    ...
    function setDefaultConfigForChainId(
        ...
        require(
            _inboundProofLibraryVersion <= maxInboundProofLibrary[_chainId] &&
                _inboundProofLibraryVersion > 0,
            "LayerZero: invalid inbound proof library version"
        );
        require(
            _inboundBlockConfirmations > 0,
            "LayerZero: invalid inbound block confirmation"
        );
        ...
        require(
            _outboundBlockConfirmations > 0,
            "LayerZero: invalid outbound block confirmation"
        );
    ...
    function setDefaultAdapterParamsForChainId(
        ...
        emit SetDefaultAdapterParamsForChainId(
            _chainId,
            _proofType,
            _adapterParams
        );
    ...
    function getOutboundNonce(
        uint16 _chainId,
        bytes calldata _path
    ) public virtual view override returns (uint64) {
