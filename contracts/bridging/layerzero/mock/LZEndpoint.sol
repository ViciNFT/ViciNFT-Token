...
contract LZEndpoint is Ownable, ILayerZeroEndpoint {
    ...
    modifier receiveNonReentrant() {
        require(
            ...
            "LayerZero: no receive reentrancy"
        );
    ...
    modifier validVersion(uint16 _version) {
        require(
            _version <= latestVersion || _version == BLOCK_VERSION,
    ...
    ) public virtual payable override sendNonReentrant {
    ...
    function receivePayload(
        ...
        uint _gasLimit,
        bytes calldata _payload
    ) public virtual override receiveNonReentrant {
        ...
        if (uaConfig.receiveVersion == DEFAULT_VERSION) {
            require(
                defaultReceiveLibraryAddress == msg.sender,
                "LayerZero: invalid default library"
            );
        } else {
            require(
                uaConfig.receiveLibraryAddress == msg.sender,
    ...
    function newVersion(
        address _newLayerZeroLibraryAddress
    ) public virtual onlyOwner {
        require(
            _newLayerZeroLibraryAddress != address(0x0),
        ...
        latestVersion++;
        libraryLookup[latestVersion] = ILayerZeroMessagingLibrary(
            _newLayerZeroLibraryAddress
        );
    ...
    ) public virtual onlyOwner validVersion(_newDefaultSendVersion) {
        require(
            _newDefaultSendVersion != DEFAULT_VERSION,
    ...
    ) public virtual onlyOwner validVersion(_newDefaultReceiveVersion) {
        require(
            _newDefaultReceiveVersion != DEFAULT_VERSION,
            "LayerZero: default receive version must > 0"
        );
        defaultReceiveVersion = _newDefaultReceiveVersion;
        defaultReceiveLibraryAddress = address(
            libraryLookup[defaultReceiveVersion]
        );
    ...
    function setConfig(
    ...
    ) public virtual override validVersion(_version) {
        ...
        libraryLookup[_version].setConfig(
            _chainId,
            msg.sender,
            _configType,
            _config
    ...
    function setSendVersion(
        uint16 _newVersion
    ) public virtual override validVersion(_newVersion) {
        // write into config
        LibraryConfig storage uaConfig = uaConfigLookup[msg.sender];
    ...
    function _getSendLibrary(
        LibraryConfig storage uaConfig
    ) internal view returns (ILayerZeroMessagingLibrary) {
        if (uaConfig.sendVersion == DEFAULT_VERSION) {
            // check if the in send-blocking upgrade
            require(
                defaultSendVersion != BLOCK_VERSION,
            ...
            // check if the in send-blocking upgrade
            require(
                uaConfig.sendVersion != BLOCK_VERSION,
    ...
    function getSendLibraryAddress(
        address _userApplication
    ) public virtual view override returns (address sendLibraryAddress) {
        LibraryConfig storage uaConfig = uaConfigLookup[_userApplication];
        uint16 sendVersion = uaConfig.sendVersion;
        require(
            sendVersion != BLOCK_VERSION,
    ...
    function getReceiveLibraryAddress(
        address _userApplication
    ) public virtual view override returns (address receiveLibraryAddress) {
        LibraryConfig storage uaConfig = uaConfigLookup[_userApplication];
        uint16 receiveVersion = uaConfig.receiveVersion;
        require(
            receiveVersion != BLOCK_VERSION,
            "LayerZero: receive version is BLOCK_VERSION"
        );
        if (receiveVersion == DEFAULT_VERSION) {
            require(
                defaultReceiveVersion != BLOCK_VERSION,
    ...
    function isSendingPayload() public virtual view override returns (bool) {
        return _send_entered_state == _ENTERED;
    }

    function isReceivingPayload() public virtual view override returns (bool) {
    ...
    function getChainId() public virtual view override returns (uint16) {
        return chainId;
    }

    function getSendVersion(
        address _userApplication
    ) public virtual view override returns (uint16) {
        LibraryConfig storage uaConfig = uaConfigLookup[_userApplication];
        return
            uaConfig.sendVersion == DEFAULT_VERSION
                ? defaultSendVersion
                : uaConfig.sendVersion;
    }
    ...
    function getReceiveVersion(
        ...
        LibraryConfig storage uaConfig = uaConfigLookup[_userApplication];
        return
            uaConfig.receiveVersion == DEFAULT_VERSION
                ? defaultReceiveVersion
                : uaConfig.receiveVersion;
    }
    ...
    function getConfig(
        ...
        address _userApplication,
        uint _configType
    ) public virtual view override validVersion(_version) returns (bytes memory) {
        if (_version == DEFAULT_VERSION) {
            require(
                defaultSendVersion == defaultReceiveVersion,
        ...
        return
            libraryLookup[_version].getConfig(
                _chainId,
                _userApplication,
                _configType
            );
