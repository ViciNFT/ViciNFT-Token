    ...
    IValidationLibraryHelperV2
{
    ...
    function validateProof(
    ...
    ) public virtual view override returns (LayerZeroPacket.Packet memory packet) {
        require(_remoteAddressSize > 0, "ProofLib: invalid address size");
        // _transactionProof = srcUlnAddress (32 bytes) + lzPacket
        require(
            _transactionProof.length > 32 &&
                keccak256(_transactionProof) == _packetHash,
        ...
        packet = LayerZeroPacket.getPacketV3(
            _transactionProof[32:],
            _remoteAddressSize,
            ulnAddress
        );
    ...
    function getUtilsVersion() public virtual view override returns (uint8) {
        return utilsVersion;
    }

    function getProofType() public virtual view override returns (uint8) {
        return proofType;
    }
    ...
    ) public virtual pure override returns (ULNLog memory log) {}

    function getPacket(
        bytes memory data,
        uint sizeOfSrcAddress,
        bytes32 ulnAddress
    ) public virtual pure override returns (LayerZeroPacket.Packet memory) {
