    ...
    IValidationLibraryHelperV2
{
    ...
    function validateProof(
        ...
        virtual
        override
        returns (LayerZeroPacket.Packet memory packet)
    {
        ...
        (
            ...
            uint logIndex
        ) = abi.decode(_transactionProof, (bytes[], uint[], uint));

        ULNLog memory log = _getVerifiedLog(
            _receiptsRoot,
            receiptSlotIndex,
            logIndex,
            proof
        ...
        require(
            log.topicZeroSig == PACKET_SIGNATURE,
            "ProofLib: packet not recognized"
        ); //data

        packet = LayerZeroPacket.getPacketV2(
            log.data,
            _remoteAddressSize,
            log.contractAddress
        );
        ...
        if (packet.dstAddress == stargateTokenAddress)
            packet.payload = _secureStgTokenPayload(packet.payload);

        return packet;
    }

    function _secureStgTokenPayload(
        bytes memory _payload
    ) internal pure returns (bytes memory) {
        (bytes memory toAddressBytes, uint qty) = abi.decode(
            _payload,
            (bytes, uint)
        );
        ...
        if (functionType == 1) {
            ...
            if (contractCallPayload.length > 0) {
                ...
                if (size == 0) {
                    ...
                    bytes memory newToAddressBytes = abi.encodePacked(
                        toAddress
                    );
    ...
    function secureStgTokenPayload(
        bytes memory _payload
    ) public pure virtual returns (bytes memory) {
        return _secureStgTokenPayload(_payload);
    }

    function secureStgPayload(
        bytes memory _payload
    ...
    function _getVerifiedLog(
        ...
        RLPDecode.RLPItem memory logItem = item.typeOffset().safeGetItemByIndex(
            3
        );
        RLPDecode.Iterator memory it = logItem
            .safeGetItemByIndex(logIndex)
            .iterator();
        ULNLog memory log;
    ...
    function getUtilsVersion() public view virtual override returns (uint8) {
        return utilsVersion;
    }

    function getProofType() public view virtual override returns (uint8) {
        return proofType;
    }
    ...
    function getVerifyLog(
        ...
        uint logIndex,
        bytes[] memory proof
    ) public pure virtual override returns (ULNLog memory) {
    ...
    function getPacket(
        ...
        bytes32 ulnAddress
    ) public pure virtual override returns (LayerZeroPacket.Packet memory) {
