...
contract RelayerV2 is ReentrancyGuard, Ownable, ILayerZeroRelayerV2 {
    ...
    function setDstPrice(
        uint16 _chainId,
        uint128 _dstPriceRatio,
        uint128 _dstGasPriceInWei
    ) public virtual onlyApproved {
    ...
    function setDstConfig(
        ...
        dstConfigLookup[_chainId][_outboundProofType] = DstConfig(
            _dstNativeAmtCap,
            _baseGas,
            _gasPerByte
        );
    ...
    function _getPrices(
        ...
        if (txType == 2) {
            ...
            require(
                dstConfig.dstNativeAmtCap >= dstNativeAmt,
                "Relayer: dstNativeAmt too large"
            );
            totalRemoteToken = totalRemoteToken.add(dstNativeAmt);
        }
        // remoteGasTotal = dstGasPriceInWei * (baseGas + extraGas)
        uint remoteGasTotal = dstPrice.dstGasPriceInWei.mul(
        ...
        pricePerByte = dstPrice
            .dstGasPriceInWei
            .mul(dstConfig.gasPerByte)
            .mul(dstPrice.dstPriceRatio)
            .div(10 ** 10);
    }
    ...
    function validateTransactionProofV1(
        ...
        bytes32 _data,
        bytes calldata _transactionProof
    ) public virtual onlyApproved nonReentrant {
        uln.validateTransactionProof(
            _srcChainId,
            _dstAddress,
            _gasLimit,
